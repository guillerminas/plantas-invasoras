...
public class Juego extends InterfaceJuego {
    ...
    public Juego() {
        ...
        laika = new Laika((double) 1024 / 2, this.entorno.ancho() - 40);
    ...
    public void tick() {
        ...
        entorno.escribirTexto("posicion en x:" + laika.x, 830, 45);
        entorno.escribirTexto("posicion en y:" + laika.y, 830, 95);
        entorno.dibujarRectangulo(1024 / 2, this.entorno.alto() - 20, 1024, 80, 0, Color.white);
    ...
    private int restriccionm(Manzana[][] m, Laika a) {
        for (int i = 0; i < fila; i++) {
            for (int j = 0; j < columna; j++)
                if (estaTocando(m[i][j], a) < 5) {
                    return estaTocando(m[i][j], a);
                }
        }
        return 5;
    }
